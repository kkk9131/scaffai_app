...
function baseWidth(width, unit = STANDARD_PART_SIZE) {
    return width - (width % unit);
}
function selectParts(targetLength, partsOptions = NORMAL_PARTS, maxItems = 4) {
    let best = null;
...
function generateCombinations(items, count) {
    if (count === 0)
        return [[]];
    if (count === 1)
...
function calculateInitialMargins(currentTotalSpan, width, leftBoundary, rightBoundary, targetMargin = DEFAULT_TARGET_MARGIN) {
...
function calculateSpanWithBoundaries(width, eaves, mandatorySpecialParts, availableNormalParts, leftBoundary, rightBoundary, targetMargin = DEFAULT_TARGET_MARGIN) {
...
function formatSpanParts(parts) {
    const count = new Map();
    parts.forEach(part => {
...
function calculateFaceDimensions(widthVal, eavesLeftVal, eavesRightVal, boundaryLeftVal, boundaryRightVal, use150Val, use300Val, use355Val, partsMasterList, targetMarginVal = DEFAULT_TARGET_MARGIN) {
    const eavesForSpanCalc = Math.max(eavesLeftVal, eavesRightVal);
    const mandatorySpecialParts = [];
    const specialParts = [[150, use150Val], [300, use300Val], [355, use355Val]];
    for (const [part, count] of specialParts) {
        for (let i = 0; i < count; i++) {
            mandatorySpecialParts.push(part);
        }
    }
    const [baseVal, partsVal, totalVal] = calculateSpanWithBoundaries(widthVal, eavesForSpanCalc, mandatorySpecialParts, partsMasterList, boundaryLeftVal, boundaryRightVal, targetMarginVal);
    const [leftMargin, rightMargin] = calculateInitialMargins(totalVal, widthVal, boundaryLeftVal, boundaryRightVal, targetMarginVal);
    ...
    let corrValForLeftNote = null;
    let corrValForRightNote = null;
...
function calcAll(input) {
    const { widthNS, widthEW, eavesN, eavesE, eavesS, eavesW, boundaryN, boundaryE, boundaryS, boundaryW, standardHeight, roofShape, tieColumn, railingCount, use355NS = 0, use300NS = 0, use150NS = 0, use355EW = 0, use300EW = 0, use150EW = 0, targetMargin = DEFAULT_TARGET_MARGIN } = input;
    // NS direction calculation (East/West gaps)
    const nsResult = calculateFaceDimensions(widthNS, eavesE, eavesW, boundaryE, boundaryW, use150NS, use300NS, use355NS, NORMAL_PARTS, targetMargin);
    // EW direction calculation (North/South gaps)
    const ewResult = calculateFaceDimensions(widthEW, eavesS, eavesN, boundaryS, boundaryN, use150EW, use300EW, use355EW, NORMAL_PARTS, targetMargin);
